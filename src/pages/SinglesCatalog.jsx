...
function FilterCheck({ label, name, value, checked, onChange }) {
  return (
    <label className="flex items-center gap-2.5 cursor-pointer py-1">
      <input
        type="checkbox" name={name} value={value} checked={checked} onChange={onChange}
...
function CardItem({ card, viewMode }) {
  ...
  const CartBtn = ({ className = '' }) => outOfStock
    ? <span className={`p-2 rounded-xl bg-white/5 text-gray-500 cursor-not-allowed ${className}`}><span className="material-symbols-outlined text-lg">remove_shopping_cart</span></span>
    : (
      <button onClick={handleAddToCart} disabled={addState !== 'idle'}
        className={`p-2 rounded-xl transition-colors flex-shrink-0 ${addState === 'done' ? 'bg-green-500 text-white' : 'bg-violet-500/10 hover:bg-violet-600 text-violet-400 hover:text-white'} ${className}`}
      >
  ...
  if (viewMode === 'grid') {
    return (
      <div className={`group bg-white/5 border border-white/10 rounded-2xl p-4 hover:shadow-xl hover:-translate-y-1 transition-all ${outOfStock ? 'opacity-60' : ''}`}>
        ...
        {outOfStock
          ? <div className="flex items-center justify-between mt-1"><span className="text-base font-black text-gray-500">Sin stock</span><CartBtn /></div>
...
function Pagination({ currentPage, total, limit, onPage }) {
  ...
  const btn = (label, page, disabled = false, active = false) => (
    <button key={label} onClick={() => !disabled && onPage(page)} disabled={disabled}
  ...
  for (const p of sorted) {
    if (prev !== null && p - prev > 1) pageButtons.push(<span key={`e-${p}`} className="w-10 h-10 flex items-center justify-center text-gray-500 text-sm">…</span>);
...
export function SinglesCatalog() {
  ...
  useEffect(() => {
    api.rarities([gameId]).then(data => setRarities(data?.rarities || data || [])).catch(() => {});
    api.sets(gameId).then(data => setAllSets(data?.sets || data || [])).catch(() => {});
    setSelectedRarities([]);
    setSelectedSets([]);
  }, [gameId]);
  ...
  }, [gameId, selectedRarities, selectedConditions, selectedSets, debouncedPrice, debouncedSearch, sort, page]);

  useEffect(() => { setPage(1); }, [selectedRarities, selectedConditions, selectedSets, debouncedPrice, debouncedSearch, sort]);
  ...
  const filteredSets = allSets.filter(s => (s.name || s.setName || '').toLowerCase().includes(setQuery.toLowerCase()));

  const FilterPanel = () => (
    <div className="space-y-2">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-bold text-white">Filtros</h2>
        <button onClick={clearAll} className="text-xs text-violet-400 hover:underline font-semibold">Limpiar todo</button>
      </div>
          ...
          {rarities.length === 0
            ...
            : rarities.map(r => {
                ...
                return <FilterCheck key={val} label={r.name || r} name="rarity" value={val}
                  checked={selectedRarities.includes(val)} onChange={() => toggleFilter(setSelectedRarities, val)} />;
              })
          }
        </div>
      </FilterGroup>
      ...
      <FilterGroup icon="payments" title="Precio">
        <div className="pt-2">
          <div className="flex justify-between text-xs text-gray-400 mb-2"><span>$0</span><span>{formatCLP(priceMax)}</span></div>
          <input type="range" min={0} max={5000000} step={10000} value={priceMax}
            onChange={e => setPriceMax(Number(e.target.value))} className="w-full accent-violet-600" />
        ...
        <div className="space-y-1 pt-1">
          {Object.entries(COND_LABELS).map(([key, label]) => (
            <FilterCheck key={key} label={label} name="condition" value={key}
              checked={selectedConditions.includes(key)} onChange={() => toggleFilter(setSelectedConditions, key)} />
          ))}
        </div>
      </FilterGroup>
      ...
      <FilterGroup icon="folder" title="Set / Expansión">
        <div className="pt-2 space-y-2">
          <div className="relative">
            ...
            <input type="text" placeholder="Buscar set..." value={setQuery} onChange={e => setSetQuery(e.target.value)}
          ...
          <div className="max-h-48 overflow-y-auto space-y-1">
            {filteredSets.map(s => {
              ...
              return <FilterCheck key={val} label={s.name || s.setName || val} name="set" value={val}
                checked={selectedSets.includes(val)} onChange={() => toggleFilter(setSelectedSets, val)} />;
            })}
          </div>
        </div>
      </FilterGroup>
    ...
    <div className="min-h-screen bg-[#0f0f0f]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <nav className="flex items-center gap-2 text-sm text-gray-500 mb-6">
          ...
          <span className="material-symbols-outlined text-base text-gray-600">chevron_right</span>
          <span className="text-white font-medium">{gameName || gameId}</span>
        </nav>

        <div className="flex items-center gap-4 mb-8">
          <Link to="/singles" className="p-2 rounded-xl bg-white/5 border border-white/10 text-gray-400 hover:text-violet-400 hover:border-violet-500 transition-colors">
            <span className="material-symbols-outlined text-base">arrow_back</span>
          </Link>
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">{gameName || gameId}</h1>
            <p className="text-gray-500 text-sm">Singles en stock</p>
          </div>
        </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-col gap-3 mb-6 bg-white/5 rounded-2xl border border-white/10 px-4 py-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <button onClick={() => setMobileOpen(true)} className="lg:hidden flex items-center gap-1.5 px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-sm text-gray-400">
                  <span className="material-symbols-outlined text-base">tune</span>Filtros
                </button>
                <div className="relative flex-1 min-w-[140px]">
                  ...
                  <input type="text" placeholder="Buscar carta..." value={search} onChange={e => setSearch(e.target.value)}
                ...
                <select value={sort} onChange={e => setSort(e.target.value)}
                  className="pl-3 pr-8 py-2 bg-[#1a1a1a] border border-white/10 rounded-xl text-sm text-gray-300 focus:outline-none focus:border-violet-500 cursor-pointer [color-scheme:dark]"
                >
                ...
                <div className="flex items-center bg-white/5 rounded-xl p-1 gap-1">
                  {['grid', 'list'].map(mode => (
                    <button key={mode} onClick={() => setViewMode(mode)}
                      className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${viewMode === mode ? 'bg-violet-600 text-white' : 'text-gray-400 hover:text-white'}`}
                    >
                      <span className="material-symbols-outlined text-base">{mode === 'grid' ? 'grid_view' : 'view_list'}</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {loading ? (
              <div className={viewMode === 'grid' ? 'grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5' : 'flex flex-col gap-3'}>
